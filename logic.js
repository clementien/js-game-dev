...
// event listener to check where u touch the screen and prevents scrolling.
...
document.addEventListener("touchend", (e)=>{
	...
	// check if the element trigerred has a class name "tile"
	if(!e.target.className.includes("tile")){
		return; // exit the function.
	} 


	// calculate the difference between the initial position & the final touch
	...
	// check if swipe is for horizontal or vertical.
	// horizontal > vertical
	...
	setTimeout(()=>{
		if(hasLost()){
			...
			// reset the game
			restartGame();
			alert("Tap the screen to restart !! ");
		}
		else{
			checkWin();
		}
